...
import static mdm.qubit.dpc.util.flags.Flags.namedParam;
import static mdm.qubit.dpc.util.flags.Flags.optional;
import static mdm.qubit.dpc.util.flags.Flags.ordinalParam;
import static mdm.qubit.dpc.util.flags.Flags.repeated;
...
import mdm.qubit.dpc.policy.SecurityLogsFragment;
import mdm.qubit.dpc.policy.resetpassword.ResetPasswordWithTokenFragment;
import mdm.qubit.dpc.util.flags.Flags;
import java.io.File;
 ...
 *
 * <p>Usage: {@code adb shell dumpsys activity --user USER_ID service mdm.qubit.dpc CMD}.
 */
        ...
        command("set-user-icon", this::setUserIcon, ordinalParam(File.class, "file"))
            .setDescription(
                "Set the user icon using the bitmap located at the given file, which must be"
                    ...
                    + " (/storage/emulated/0/Android/data/mdm.qubit.dpc/files/Pictures/UserIcons),"
                    ...
                    + " content://mdm.qubit.dpc.usericoncontentprovider/icon.png <"
                    + " /tmp/icon.png`)."));
    flags.addCommand(
        command(
